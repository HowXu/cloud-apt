<template>
  <div class="max-w-1280px mx-auto px-4 py-6 sm:px-8 sm:py-8">
    <header class="flex flex-wrap items-baseline justify-between mb-6 sm:mb-8 gap-2 sm:gap-3">
      <div class="flex items-baseline gap-3 min-w-0">
        <img
          v-if="siteConfig.iconUrl"
          :src="siteConfig.iconUrl"
          :alt="siteConfig.iconAlt || ''"
          class="h-7 w-7 self-center"
        />
        <h1 class="text-xl sm:text-2xl truncate">
          <router-link to="/" class="text-fg no-underline">{{ siteConfig.headerTitle }}</router-link>
        </h1>
      </div>
      <span v-if="siteConfig.headerTagline" class="text-muted text-sm">{{ siteConfig.headerTagline }}</span>
    </header>

    <SearchBox placeholder="Search packages, descriptions, dependencies…" />

    <section class="mt-6 sm:mt-8">
      <h2 class="text-base sm:text-lg mb-3 sm:mb-4">Welcome</h2>
      <p v-for="(line, i) in siteConfig.homepageIntroLines" :key="i" class="text-muted mb-2">
        {{ line }}
      </p>
    </section>

    <section class="mt-6 sm:mt-8">
      <h2 class="text-base sm:text-lg mb-3 sm:mb-4">Browse</h2>
      <router-link
        :to="`/browse?suite=${siteConfig.defaultSuite}`"
        class="inline-block px-4 py-2 bg-card border border-border rounded-md text-accent no-underline hover:bg-hover"
      >
        {{ siteConfig.defaultSuite }}
      </router-link>
    </section>

    <section v-if="siteConfig.usageSteps.length" class="mt-6 sm:mt-8">
      <h2 class="text-base sm:text-lg mb-3 sm:mb-4">How to use this repository</h2>
      <ol class="list-decimal pl-5 sm:pl-6 space-y-3">
        <li v-for="step in siteConfig.usageSteps" :key="step.title">
          <strong>{{ step.title }}</strong>
          <pre class="mt-1 p-2 bg-card border border-border rounded text-sm overflow-x-auto"><code>{{ step.code }}</code></pre>
          <p v-if="step.note" class="text-muted text-sm mt-1">{{ step.note }}</p>
        </li>
      </ol>
    </section>

    <Footer />
  </div>
</template>

<script setup lang="ts">
import { siteConfig } from '../site.config';
import SearchBox from '../components/SearchBox.vue';
import Footer from '../components/Footer.vue';
</script>