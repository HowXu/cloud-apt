"""Real GPG/reprepro tests with isolated repository and simulated upload failures."""
import contextlib
import io
import http.server
import json
...
class RepositoryTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for command in ('gpg', 'gpgconf', 'reprepro', 'dpkg-deb'):
    ...
    def prepare(self, version='1.0', mode='include'):
        operation = {'mode': mode}
        if mode == 'include':
            operation['deb'] = str(self.deb(version))
        elif mode == 'remove':
    ...
    def test_upload_disconnect_resumes_without_reprepro_or_original_deb(self):
        ...
        self.assertEqual(sum('/.snapshots/' not in key and key.startswith('pool/') for key in self.remote.calls), 2)
    ...
    def test_signed_by_hash_indexes_and_old_files_survive_upgrade_and_remove(self):
        ...
        release = (snapshot / 'indexes/Release').read_text()
        self.assertIn('Acquire-By-Hash: yes', release)
        with signing_home(self.root, PASSWORD) as (home, _):
            signed_text = gpg(home, ['--decrypt', snapshot / 'indexes/InRelease']).decode()
    ...
    def test_full_sync_reuploads_unchanged_local_files_and_rebases_conflict(self):
        snapshot = self.prepare()
        self.remote.active = 'other-writer'
        with self.assertRaises(RuntimeError):
            resume(snapshot, self.remote)
        synced = self.prepare(mode='sync')
        resume(synced, self.remote)
        self.remote.files.clear()
        resume(self.prepare(mode='sync'), self.remote)
        self.assertTrue(any(key.startswith('pool/') for key in self.remote.files))
    ...
    def test_real_apt_verifies_signatures_uses_by_hash_and_downloads_deb(self):
        ...
        self.assertEqual(sha256(downloaded), sha256(next(self.root.glob('pool/**/*.deb'))))
